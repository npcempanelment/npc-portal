 ...
 * - Pre-submission screening preview on review step
 *
...
import { previewScreening } from '../utils/clientScreening';
...
export default function ContractualApplicationPage() {
  usePageTitle('Apply for Contractual Position');
  ...
  // Pre-submission screening preview
  const screeningPreview = useMemo(() => {
    if (!advert) return null;
    return previewScreening(dob, educations, experiences, {
      maxAge: advert.maxAge, minExperienceYears: advert.minExperienceYears,
      minQualification: advert.minQualification, designation: advert.designation,
    });
  }, [dob, educations, experiences, advert]);
  ...
  if (loadingAdvert) return <div style={S.container}><p><span className="spinner" /> Loading advertisement details...</p></div>;
  ...
  if (!user) return <div style={S.container}><h2>Apply for Contractual Position</h2><div style={S.error}>Please <a href="/login">login</a> or <a href="/register">register</a> to apply.</div></div>;
      ...
      <div style={S.container}>
        ...
        {empanelOptIn && (
          <div style={{ background: '#e8f5e9', border: '1px solid #a5d6a7', padding: '12px 16px', borderRadius: 6, marginTop: 16, fontSize: '0.9rem', color: '#2e7d32' }}>
            Your empanelment application has also been submitted. It will be reviewed separately.
          </div>
        )}
        <div style={{ display: 'flex', gap: 12, marginTop: 24, flexWrap: 'wrap' }}>
          <Link to="/my-applications" style={S.nextBtn}>View My Applications</Link>
          <Link to="/adverts" style={S.backBtn}>Browse More Positions</Link>
          <Link to="/dashboard" style={S.backBtn}>Go to Dashboard</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="page-container" style={S.container}>
      ...
      {profileLoaded && <div style={S.prefillBanner}>Your previous profile data has been pre-filled. Review and update if needed.</div>}

      <div style={S.stepBar}>
        {STEP_LABELS.map((label, idx) => (
          <div key={idx} style={{ ...S.stepItem, background: step === idx ? '#1a237e' : step > idx ? '#4caf50' : '#e0e0e0', color: step >= idx ? '#fff' : '#666' }}>
            <span style={{ fontWeight: 'bold', marginRight: 6 }}>{idx + 1}</span>{label}
          </div>
        ))}
      </div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        {step === 0 && (
          <div>
            <div style={S.advertBox}>
              ...
              {advert.eligibilityCriteria && <div style={{ marginTop: 12 }}><strong>Eligibility Criteria:</strong><div style={{ fontSize: '0.9rem', marginTop: 4, whiteSpace: 'pre-line' as const }}>{advert.eligibilityCriteria}</div></div>}
              {advert.description && <p style={{ marginTop: 12, fontSize: '0.9rem', color: '#333' }}>{advert.description}</p>}
            </div>
            <div style={S.note}>Please fill in all details in the following steps. Your profile will be auto-screened against the eligibility criteria as per AI-858/2026.</div>
          </div>
        )}
        ...
        {step === 1 && (
          <fieldset style={S.fieldset}>
            ...
            <div className="form-row" style={S.row}>
              <label style={S.label}>Gender<select value={gender} onChange={e => setGender(e.target.value)} style={S.input}><option value="">-- Select --</option>{GENDER_OPTIONS.map(g => <option key={g} value={g}>{g}</option>)}</select></label>
        ...
        {step === 2 && (
          <fieldset style={S.fieldset}>
            ...
            {educations.map((edu, idx) => { const tag = `EDU-${idx + 1}`; return (
        ...
        {step === 3 && (
          <>
            <fieldset style={S.fieldset}>
              ...
              {experiences.map((exp, idx) => { const tag = `EXP-${idx + 1}`; return (
                <div key={idx} style={S.entryBlock}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            ...
            <fieldset style={S.fieldset}>
              ...
              {certifications.map((cert, idx) => { const tag = `CERT-${idx + 1}`; return (
                <div key={idx} style={S.entryBlock}>
        ...
        {step === 4 && (
          <>
            ...
            {screeningPreview && (
              <fieldset style={{
                ...S.fieldset,
                border: screeningPreview.eligible ? '2px solid #4caf50' : '2px solid #ff9800',
                background: screeningPreview.eligible ? '#f1f8e9' : '#fff8e1',
              }}>
                <legend style={{ ...S.legend, color: screeningPreview.eligible ? '#2e7d32' : '#e65100' }}>
                  {screeningPreview.eligible ? 'Eligibility Preview — You Appear Eligible' : 'Eligibility Preview — Potential Issues Found'}
                </legend>
                <p style={{ fontSize: '0.85rem', color: '#555', margin: '0 0 12px' }}>
                  This is a preliminary check based on your data. The official screening happens after submission.
                  {!screeningPreview.eligible && ' You can go back to correct your data, or submit as-is for committee review.'}
                </p>
                <div className="screening-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 12 }}>
                  <div style={S.screeningCard}>
                    <div style={{ fontSize: '0.8rem', color: '#666', marginBottom: 4 }}>Age</div>
                    <div style={{ fontSize: '1.1rem', fontWeight: 'bold' }}>{screeningPreview.age} years</div>
                    <div style={{ color: screeningPreview.meetsAge ? '#2e7d32' : '#c62828', fontWeight: 600, fontSize: '0.85rem', marginTop: 4 }}>
                      {screeningPreview.meetsAge ? 'Within limit' : `Exceeds max ${advert.maxAge || 65}`}
                    </div>
                  </div>
                  <div style={S.screeningCard}>
                    <div style={{ fontSize: '0.8rem', color: '#666', marginBottom: 4 }}>Experience</div>
                    <div style={{ fontSize: '1.1rem', fontWeight: 'bold' }}>{screeningPreview.totalExperience} yrs</div>
                    <div style={{ color: screeningPreview.meetsExperience ? '#2e7d32' : '#c62828', fontWeight: 600, fontSize: '0.85rem', marginTop: 4 }}>
                      {screeningPreview.meetsExperience ? `Meets min ${advert.minExperienceYears || 0}` : `Below min ${advert.minExperienceYears || 0}`}
                    </div>
                  </div>
                  <div style={S.screeningCard}>
                    <div style={{ fontSize: '0.8rem', color: '#666', marginBottom: 4 }}>Qualification</div>
                    <div style={{ fontSize: '1.1rem', fontWeight: 'bold' }}>{screeningPreview.qualificationLabel}</div>
                    <div style={{ color: screeningPreview.meetsQualification ? '#2e7d32' : '#c62828', fontWeight: 600, fontSize: '0.85rem', marginTop: 4 }}>
                      {screeningPreview.meetsQualification ? 'Meets requirement' : 'Below requirement'}
                    </div>
                  </div>
                </div>
                {screeningPreview.issues.length > 0 && (
                  <div style={{ background: '#fff3e0', padding: '10px 14px', borderRadius: 6, border: '1px solid #ffe0b2' }}>
                    <strong style={{ color: '#e65100', fontSize: '0.85rem' }}>Issues found:</strong>
                    <ul style={{ margin: '6px 0 0', paddingLeft: 20 }}>
                      {screeningPreview.issues.map((issue, i) => <li key={i} style={{ fontSize: '0.85rem', color: '#bf360c', marginBottom: 4 }}>{issue}</li>)}
                    </ul>
                    <p style={{ margin: '8px 0 0', fontSize: '0.85rem', color: '#e65100' }}>
                      You can <strong>go back</strong> to correct your details, or proceed to submit for committee review.
                    </p>
                  </div>
                )}
              </fieldset>
            ...
            <fieldset style={{ ...S.fieldset, border: '2px solid #c5cae9', background: '#f3f4fb' }}>
              ...
              <label style={{ fontSize: '0.9rem', display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: 12 }}>
                ...
                <span><strong>Yes, I would also like to be empaneled with NPC.</strong> Your same profile will be registered for empanelment too, allowing NPC to engage you on short notice. Valid for 3 years.</span>
              </label>
              {empanelOptIn && (
                <div style={{ padding: '12px 0 0', borderTop: '1px solid #ddd' }}>
                  <div className="form-row" style={S.row}>
                    <label style={S.label}>Domain *<select value={empDomainId} onChange={e => { setEmpDomainId(e.target.value); setEmpSubDomainId(''); }} style={S.input}><option value="">-- Select Domain --</option>{domains.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}</select></label>
                    <label style={S.label}>Sub-Domain<select value={empSubDomainId} onChange={e => setEmpSubDomainId(e.target.value)} style={S.input} disabled={!empDomainId}><option value="">-- Select --</option>{domains.find(d => d.id === empDomainId)?.subDomains.map(sd => <option key={sd.id} value={sd.id}>{sd.name}</option>)}</select></label>
                  </div>
                  <label style={S.label}>Empanelment Area *<select value={empArea} onChange={e => setEmpArea(e.target.value)} style={S.input}>{EMPANELMENT_AREAS.map(ea => <option key={ea.value} value={ea.value}>{ea.label}</option>)}</select></label>
                  <div style={{ marginTop: 8 }}>
                    ...
                    <div className="checkbox-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 250px), 1fr))', gap: 4, marginTop: 6 }}>
                      {allOffices.map(o => <label key={o.id} style={S.checkLabel}><input type="checkbox" checked={empOfficeIds.includes(o.id)} onChange={() => setEmpOfficeIds(prev => prev.includes(o.id) ? prev.filter(x => x !== o.id) : [...prev, o.id])} />{o.city}</label>)}
                    </div>
                  </div>
                </div>
              )}
            ...
            <div style={{ marginBottom: 16 }}>
              <label style={{ fontSize: '0.9rem', display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                ...
                <span>I confirm that the information provided is accurate and complete. I have read the terms & conditions of contractual engagement as per AI-858/2026. I understand that my profile will be auto-screened against the eligibility criteria.{empanelOptIn && ' I also consent to empanelment with NPC using the same profile.'}</span>
              </label>
            </div>
          </>
        )}
        ...
        <div style={S.navRow}>
          ...
          <div style={{ flex: 1 }} />
          {step < 4 ? (
            <button type="button" onClick={goNext} style={S.nextBtn}>{step === 0 ? 'Start Application' : 'Next'}</button>
          ) : (
            <button type="submit" disabled={loading || !confirmed} style={{ ...S.submitBtn, opacity: confirmed ? 1 : 0.5 }}>
              {loading ? <><span className="spinner spinner-light" /> Submitting...</> : 'Submit Application'}
            </button>
          )}
        </div>
      </form>
...
const S: Record<string, React.CSSProperties> = {
  ...
  prefillBanner: { background: '#e8f5e9', border: '1px solid #a5d6a7', padding: '10px 14px', borderRadius: 6, marginBottom: 16, fontSize: '0.9rem', color: '#2e7d32' },
  ...
  addBtn: { background: '#e8eaf6', border: '1px solid #9fa8da', padding: '8px 18px', borderRadius: 6, cursor: 'pointer', fontSize: '0.85rem', fontWeight: 500 },
  removeBtn: { background: '#ffebee', border: '1px solid #ef9a9a', padding: '8px 16px', borderRadius: 6, cursor: 'pointer', fontSize: '0.8rem', color: '#c62828' },
  docRow: { marginTop: 10, padding: '8px 12px', background: '#e3f2fd', borderRadius: 6, border: '1px dashed #90caf9', display: 'flex', flexWrap: 'wrap' as const, alignItems: 'center', gap: 10 },
  ...
  backBtn: { padding: '10px 24px', background: '#f5f5f5', border: '1px solid #ccc', borderRadius: 6, cursor: 'pointer', fontSize: '0.95rem', textDecoration: 'none', color: '#333', display: 'inline-block', textAlign: 'center' as const },
  nextBtn: { padding: '10px 32px', background: '#1a237e', color: '#fff', border: 'none', borderRadius: 6, cursor: 'pointer', fontSize: '0.95rem', textDecoration: 'none', display: 'inline-block', textAlign: 'center' as const },
  submitBtn: { padding: '12px 40px', background: '#1a237e', color: '#fff', border: 'none', borderRadius: 6, cursor: 'pointer', fontSize: '1rem', fontWeight: 'bold' },
  error: { background: '#ffebee', color: '#c62828', padding: '12px 14px', borderRadius: 6, marginBottom: 16, fontSize: '0.9rem' },
  note: { background: '#fff3e0', padding: 12, borderRadius: 6, border: '1px solid #ffe0b2', fontSize: '0.9rem', marginBottom: 12 },
  hint: { fontSize: '0.85rem', color: '#666', margin: '0 0 12px' },
  th: { padding: '10px 12px', borderBottom: '2px solid #c5cae9' },
  td: { padding: '8px 12px', borderBottom: '1px solid #eee' },
  screeningCard: { background: '#fff', border: '1px solid #ddd', borderRadius: 8, padding: 14, textAlign: 'center' as const },
};
